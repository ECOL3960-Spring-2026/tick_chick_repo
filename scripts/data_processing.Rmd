...
daily_temp_data <- temperature_data |> 
  ...
  filter(!is.na("mean_temp") & !is.na("max_temp") & !is.na("min_temp"))
...
```{r, calculate monthly temperature data, warning=FALSE}
...
abundance_data <- abundance_data %>% 
  select(siteID, plotID, nlcdClass, elevation, collectDate, sampleID,
         adultCount, nymphCount, larvaCount) %>%
  mutate(
  date = as_date(collectDate),
) %>%
  filter(!is.na("sampleID") & !is.na("adultCount") & !is.na("nymphCount") & !is.na("larvaCount"))
...
pathogen_status_data <- pathogen_status_data %>% 
  select(siteID, plotID, nlcdClass, elevation, collectDate, individualCount, testResult, 
